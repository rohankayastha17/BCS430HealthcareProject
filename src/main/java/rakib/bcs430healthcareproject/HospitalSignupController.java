...
public class HospitalSignupController {
    ...
    @FXML
    private void onCreateHospitalAccount() {
        String hospitalName = safe(hospitalNameField);
        ...
        if (hospitalName.isEmpty() || email.isEmpty() || password.isEmpty()
        ...
        showInfo("Creating hospital account and claiming location...");

        firebaseService.createHospital(email, password, hospitalName, phone, address, city, state, zip)
                .thenAccept(uid -> Platform.runLater(() -> {
                    showSuccess("Hospital account created. Please sign in to access the portal.");
